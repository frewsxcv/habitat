...
pub fn job_group_cancel(
    ...
    // Switch all the NotStarted projects to Cancelled

    // Send Cancel messages to all the InProgress projects
    for project in group.get_projects().iter().filter(|&ref p| {
        p.get_state() == jobsrv::JobGroupProjectState::InProgress
    })
    {
        println!("Cancelling: {:?}", project);
    }

    // Set the Group state to Cancelled
